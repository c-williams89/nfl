...
END_TEST START_TEST(test_llist_enqueue_valid)
{
	llist_t *llist = llist_create();
	int x = 0;
	void *data = &x;
	ck_assert_int_eq(llist_enqueue(llist, data), 1);
} END_TEST START_TEST(test_llist_enqueue_invalid)
{
...
} END_TEST START_TEST(test_llist_dequeue_valid)
{
	int num_elements = 20;
...
} END_TEST START_TEST(test_llist_dequeue_invalid)
{
	llist_t *llist = NULL;
...
} END_TEST START_TEST(test_llist_is_empty)
{
	llist_t *llist = NULL;
	...
	ck_assert_int_eq(llist_is_empty(llist), 1);
} END_TEST START_TEST(test_llist_create_iter)
{
	llist_t *llist = NULL;
	ck_assert_int_eq(llist_create_iter(llist, NULL), 0);
	llist = llist_create();
	ck_assert_int_eq(llist_create_iter(llist, NULL), 0);
	llist_iter_t iter = { 0 };
	ck_assert_int_eq(llist_create_iter(llist, &iter), 0);
	char string[] = "Test iter";
	void *data = string;
	llist_enqueue(llist, data);
	ck_assert_int_ne(llist_create_iter(llist, &iter), 0);

} END_TEST START_TEST(test_llist_iter_next)
{
	ck_assert_ptr_eq(llist_iter_next(NULL), NULL);
} END_TEST TFun llist_tests[] = {
